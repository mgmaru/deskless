...
| [Dockerで何を隔離するか](#dockerで何を隔離するか) | 隔離する理由／コンテナから見える範囲／守られるもの・守られないもの |
...
並列タスクは、worktreeごとにtmuxのウィンドウを分けて扱う。
...
コンテナは自分の mount namespace を持つため、中で `rm -rf /usr` をしてもコンテナが壊れるだけで、WSLのdistro本体には届かない。想定している「エージェントが暴走して環境を壊す」は、これで防げる。

ただし**「見えない」と「到達できない」は別**であり、分離されている層とされていない層がある。

**見えないもの**

| 対象 | 理由 |
| --- | --- |
| distro の `/usr` `/etc` `/home` | コンテナに見えるのはイメージの中身であり、distroのファイルはmount namespaceに**存在しない** |
| distro に apt で入れたパッケージ | 同上。コンテナ内の `apt list` は別世界 |
| ホスト側のプロセス | PID namespace が分離されている（`--pid=host` を付けない限り） |
| `/mnt/c` | WSL distro 側のマウントであり、コンテナには引き継がれない |
...
2行目はこの構成では具体的な意味を持つ。[Phase 3](#phase-3pcからの窓口とネットワークを繋ぐ) でWSL上にsshdを立てるため、**コンテナからそのsshdに到達できる**。鍵を渡していないので入れないが、経路が無いわけではない。

まとめると、**ファイルシステムとしては見えない。しかしdistroは「隣にいる」**。

したがって脅威の想定によって、対処すべき場所が変わる。

- **操作ミス・暴走**を想定するなら、見えない＝壊せない、で足りる
- **悪意あるコード**（依存パッケージ、プロンプトインジェクション）まで想定を広げると、共有カーネルとネットワーク経路が残る。ここは箱を厚くするのではなく、**資格情報を持たせない**（`~/.ssh` を渡さない、push権限を分離する）で対処する

### 境界を決めるのはマウント

カーネルとネットワークは上記の通り共有されるが、**ファイルに関する境界はマウントの有無だけ**で決まる。

bind mountしたパスについては、コンテナ内のrootはホスト上でもuid 0として書き込む。マウントした範囲の中身は削除もchownもできる。ただしマウント外のファイルには手が出ない。

したがって「Dockerに入れたから安全」ではなく、「マウントしなかったものが守られる」と理解する。広くマウントする設計なら、隔離の効果はその分だけ失われる。

### マウント方針

```
WSL (Ubuntu)
├─ ~/work/<repo>          ← リポジトリはホスト側に置く。Zedはここを直接見る
├─ ~/.claude              ← 認証情報。これ「だけ」渡す
└─ tmux
    └─ docker exec
        └─ コンテナ
            /workspace       ← ~/work/<repo> を bind mount
            /root/.claude    ← ~/.claude を bind mount
```

渡すのはこの2つだけ。`~/.ssh` も `$HOME` 全体も渡さない。
...
| **守られない** | `/workspace` の中身 | マウントしている。ただしgitで復元できる |
...
コンテナ内cloneで追加的に守れるのは4だけで、その対価としてZedからの編集経路を失う（Zedのdevcontainer対応はVS Codeほど厚くないため、コンテナ内にsshdを立てて直接接続するなどの手当てが必要になる）。割に合わない。
